...
function ProductsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const bestSeller = productList.reduce(
      (best, product) => (product.sold > best.sold ? product : best),
      productList[0],
    );
  ...
  async function handleCreate(product: Product) {
    try {
      const saved = await saveProduct(product);
      setProductList((items) => [...items, saved]);
  ...
  async function handleDelete(product: Product) {
    try {
      ...
      setProductList((items) => items.filter((item) => item.id !== product.id));
    ...
    <AppShell role="owner">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          delta="Sem data de venda"
        />
        <StatCard
          label="Estoque baixo"
          value={`${stats.lowStock} itens`}
          delta="Repor em breve"
          icon={AlertTriangle}
        />
                ...
                <td className="px-6 py-4">
                  <div className="flex justify-end gap-3">
                    <button
                      ...
                      className="text-xs text-gold hover:text-gold-hover transition"
                    >
                      Editar
                    </button>
                    <button
