...
const customerStorageKey = "maison-lame-customers";
const historyStorageKey = "maison-lame-customer-history";
...
export const customerSeed: Customer[] = [
  ...
  },
  {
    ...
    avatar_url: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=160&q=80",
    favorite_barber: "Lucas Moreau",
    favorite_cut: "Experiencia Maison",
...
export const customerHistorySeed: CustomerHistory[] = customerSeed.flatMap((customer) =>
  Array.from({ length: Math.min(customer.visits, 4) }, (_, index) => ({
    ...
    visited_at: index === 0 ? customer.last_visit || "2026-05-01" : `2026-0${Math.max(1, 5 - index)}-1${index}`,
...
export async function listCustomerHistory(customerId?: string) {
  if (supabase) {
    let query = supabase.from("customer_history").select("*").order("visited_at", { ascending: false });
    if (customerId) query = query.eq("customer_id", customerId);
    const { data, error } = await query;
    if (!error && data) return data as CustomerHistory[];
  }
...
export async function updateCustomer(customer: Customer) {
  ...
  const customers = readLocal(customerStorageKey, customerSeed);
  writeLocal(customerStorageKey, customers.map((item) => (item.id === id ? customer : item)));
  return customer;
}
...
export async function deleteCustomer(customerId: string) {
  ...
  const customers = readLocal(customerStorageKey, customerSeed);
  writeLocal(customerStorageKey, customers.filter((item) => item.id !== customerId));
}
